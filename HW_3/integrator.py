...
def rk4_step(dt,w,m):
    ...
    k1 = f_Nbody_3d(w, m)
    h_2 = dt/2
    k2 = f_Nbody_3d(w + h_2*k1, m)
    k3 = f_Nbody_3d(w + h_2*k2, m)
    k4 = f_Nbody_3d(w + dt*k3, m)
...
@njit
def adaptive_step(dt, w, m, tol, dt_guess):
    """
    ...
    while t < dt:
        ...
        # Take a full step
        w1 = rk4_step(dt_current, w1, m)

        # Take two half steps
        w2 = rk4_step(dt_current / 2, w2, m)
        w2 = rk4_step(dt_current / 2, w2, m)
...
def evolve_system(dt, t_final, w0, m,tol):
    ...
    for i in range(1, n_steps):
        ...
        w[i], dt_guess = adaptive_step(dt, w[i - 1], m, tol, dt_guess)
        dt_guesses[i] = dt_guess
        #print progress every 10% of the way
        if i % (n_steps // 50) == 0:
            prog = i / n_steps * 100
            prog = np.round(prog, 1)
            print(f'\r{prog} % done', end='')
